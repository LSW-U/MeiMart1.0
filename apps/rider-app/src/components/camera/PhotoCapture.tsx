    ...
    <View className="w-full items-center gap-6">
      <View className="aspect-[3/4] w-full max-w-sm overflow-hidden rounded-lg border border-outline-variant bg-[#eed4d1] shadow-inner">
        {captured && photoUri ? (
          ...
          <View className="h-full w-full items-center justify-center">
            <View className="h-3/5 w-4/5 items-center justify-center rounded-lg border-2 border-dashed border-white/70 bg-on-surface/20">
      ...
      <View className="w-full max-w-sm gap-2">
        <View className="flex-row items-center gap-2 px-2">
          <Text className="text-sm text-outline">i</Text>
          <Text className="flex-1 text-xs font-bold uppercase tracking-wider text-on-surface-variant">{instruction}</Text>
        </View>
        {captured && photoUri ? (
          <View className="h-24 flex-row items-center gap-4 rounded-lg border border-dashed border-outline-variant bg-surface-variant px-4">
            ...
            <View className="flex-1">
              <Text className="text-xs font-bold uppercase tracking-wider text-on-surface">{fileName}</Text>
              <Text className="text-sm text-outline">{readyLabel}</Text>
            </View>
            {onRetake ? (
              <Pressable
                accessibilityRole="button"
                className="rounded-full border border-outline-variant bg-white px-3 py-1.5 active:opacity-70"
                onPress={handleRetakePress}
              >
                <Text className="text-xs font-bold uppercase tracking-wider text-primary">{retakeLabel ?? 'Retake'}</Text>
              </Pressable>
            ) : null}
          </View>
        ) : (
          <View className="flex-row gap-3">
            <Pressable className="flex-1 items-center rounded-lg bg-primary py-3" onPress={() => void takePhoto()}>
              <Text className="text-xs font-bold text-white">CAMERA</Text>
            </Pressable>
            <Pressable className="flex-1 items-center rounded-lg border border-outline-variant bg-white py-3" onPress={() => void pickFromGallery()}>
              <Text className="text-xs font-bold text-primary">GALLERY</Text>
            </Pressable>
          </View>
        )}
      </View>
    ...
    <View className="w-full items-center gap-6">
      ...
      <View className="w-full max-w-sm gap-2">
        ...
        {captured && photoUri ? (
          ...
          <View className="flex-row gap-3">
            <Pressable className="flex-1 items-center rounded-lg bg-primary py-3" onPress={() => inputRef.current?.click()}>
              <Text className="text-xs font-bold text-white">CAMERA</Text>
            </Pressable>
            <Pressable
              className="flex-1 items-center rounded-lg border border-outline-variant bg-white py-3"
              onPress={() => {
