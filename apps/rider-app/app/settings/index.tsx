...
type SettingsItemProps = {
  icon: 'language' | 'bell' | 'shield' | 'help';
  title: string;
  description: string;
  onPress?: () => void;
...
function SettingsItem({ icon, title, description, onPress, trailing = 'chevron', switchValue = false, onSwitchChange, disabled = false }: SettingsItemProps) {
  return (
    <Pressable accessibilityRole="button" accessibilityLabel={title} accessibilityState={disabled ? { disabled: true } : undefined} className={`flex-row items-center gap-4 px-5 py-4 ${disabled ? 'opacity-60' : 'active:bg-surface-container-low'}`} disabled={disabled} onPress={onPress}>
      ...
      {trailing === 'switch' ? <Switch accessibilityRole="switch" accessibilityLabel={title} accessibilityState={{ checked: switchValue }} disabled={disabled} onValueChange={onSwitchChange} value={switchValue} /> : <AppIcon color={colors.outline} name="chevronRight" />}
    </Pressable>
  );
}

// P3-① 闪中文本页缓解：loading 期不渲染中文实体文案，改用骨架条占位
function SettingsItemSkeleton({ icon, trailing }: { icon: 'language' | 'bell' | 'shield' | 'help'; trailing?: 'switch' | 'chevron' }) {
  return (
    <View className="flex-row items-center gap-4 px-5 py-4">
      ...
      {trailing === 'switch' ? <View className="h-7 w-12 rounded-full bg-surface-container" /> : null}
    </View>
  );
}
      ...
      <ScrollView contentContainerClassName="gap-5 px-5 py-6 pb-12">
        {isError ? (
          ...
          <View className="items-center rounded-3xl bg-primary p-6 shadow-sm">
            ...
            <Pressable accessibilityRole="button" accessibilityLabel={t('settings.retry')} className="mt-4 rounded-full bg-white px-6 py-2" onPress={() => void refetch()}>
