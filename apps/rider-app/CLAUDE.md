...
1. **每个 Task 完成后**，保存执行日志到 Obsidian vault：
   - 路径：`_inbox/02-客户端记录/claude骑手端优化/补充任务清单/日志/优化流程工作-第N批/`，文件名 `✅ 已完成-描述-日期.md`
   - 区分：**总任务总清单**（如页面 UI 优化总清单）→ `claude骑手端优化/` 根；**补充任务/计划/清单**（工作前规划）→ `补充任务清单/`；**执行日志**（工作后产生）→ `补充任务清单/日志/`；**审查报告** → `补充任务清单/审查报告/`
...
| P2 | 硬编码颜色 | 🔶 组件层 ✅（`1b4c85e` 7 组件）+ 页面层第一批（49 文件 className 高频 #hex → tailwind token）；剩 inline #hex（placeholderTextColor/color 属性）+ 无映射值（status/特殊色，待评估） |
