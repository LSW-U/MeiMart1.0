...
| P2 | a11y 覆盖率低 | 🔶 四批完工（UI 库 5 + 核心路径 10 + login/profile 13 + notifications/earnings/settings 9 = 37 交互元素）；剩 ~9 页面级 Pressable（order/history + terms/privacy/help/register/profile.edit/order[id]/earnings.withdraw 散落页）+ AppIcon ~86 调用方未传 label + Text onPress 链接 |
