...
function MenuItem({ label, tone = 'default', onPress }: MenuItemProps) {
    ...
    <Pressable className={`flex-row items-center justify-between p-5 ${danger ? 'active:bg-[#ffdad6]/30' : 'active:bg-[#fff0ee]'}`} onPress={onPress}>
      <View className="flex-row items-center gap-4">
        <View className={`h-10 w-10 items-center justify-center rounded-full ${danger ? 'bg-[#ffdad6]/50' : 'bg-[#ffe9e6]'}`}>
          <Text className={danger ? 'text-[#ba1a1a]' : 'text-[#59413d]'}>{danger ? '!' : '•'}</Text>
        </View>
...
export default function ProfilePage() {
  ...
  return (
    <ScrollView className="flex-1 bg-[#fff8f7]" contentContainerClassName="pb-12">
      <View className="sticky top-0 z-50 flex-row items-center justify-between bg-[#fff8f7]/90 px-5 py-2">
        ...
        <Pressable className="rounded-full p-2" onPress={() => router.push('/profile/edit')}>
          <Text className="text-[#720003]">EDIT</Text>
        </Pressable>
      </View>
      ...
      <View className="items-center gap-4 px-5 pb-2 pt-4">
        <View className="relative items-center">
          <View className="h-24 w-24 rounded-full bg-[#634700] p-[3px]">
            <Image className="h-full w-full rounded-full border-2 border-[#fff8f7]" resizeMode="cover" source={{ uri: avatarUri }} />
          </View>
        ...
        <View className="mt-2 items-center">
          <Text className="text-2xl font-bold text-[#261816]">{t('profile.name')}</Text>
          <View className="mt-1 flex-row items-center gap-2">
            <Text className="text-sm text-[#59413d]">{t('profile.id')}</Text>
      ...
      <View className="mx-5 overflow-hidden rounded-[20px] border border-[#ffe9e6] bg-white shadow-sm">
        <MenuItem label={t('profile.earningsHistory')} onPress={() => router.push('/(main)/earnings')} />
        <View className="mx-5 h-px bg-[#e1bfba]/40" />
        <MenuItem label={t('profile.settings')} onPress={() => router.push('/settings')} />
        <View className="mx-5 h-px bg-[#e1bfba]/40" />
        <MenuItem label={t('profile.helpCenter')} />
        <View className="mx-5 h-px bg-[#e1bfba]/40" />
        <MenuItem label={t('profile.logout')} tone="danger" />
      </View>
    </ScrollView>
  );
}
